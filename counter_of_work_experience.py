...
def count_nodes(level_number):
    ...
    3
    """
    return round(((1 + level_number) / 2) * level_number)
...
def count(graph, nodes_values, level_num):
    ...
    for i in end_nodes:
        for j in find_all_paths(graph, 0, i):
            work_experience = 0
            for k in j:
                work_experience += nodes_values[k]
            if work_experience > biggest_work_experience:
                biggest_work_experience = work_experience
    return biggest_work_experience
